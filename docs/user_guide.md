...
3. Suppress small variants overlapping PASS pbsv structural variants (haplotype-aware). This step addresses the known gray area in variant size and representation where small-variant genotypers and structural-variant genotypers may both emit overlapping calls for the same underlying event, leading to redundant or conflicting genotypes in the merged VCF. During per-gene filtering, the pipeline identifies PASS pbsv structural variants that overlap small variants (SNPs or indels) from bcftools or DeepVariant. A small variant is suppressed if it overlaps a pbsv structural variant on the reference and both variants are assigned to the same haplotype (i.e., the small variant must be phased). This prevents redundant or conflicting variant calls within regions affected by structural variation. This logic was only applied to pbsv SV genotypes that are non-symbolic, FILTER=PASS, and phased or homozygous ALT. Small variants suppressed by this logic are written to SAMPLE_GENE_sv_overlap.vcf.gz. These records are excluded from downstream processing (including vcf2fasta).
