...
Another bug is that vcf2fasta checks the first genotype of the overall vcf to determine phase status. So if the first variant is unphased, it treats the whole vcf as unphased and won’t output phased sequences in FASTA format. Instead, it will output one diploid sequence with IUPAC codes representing heterozygous bases. To work around this, hla_resolve/reconstruct_fasta_methods.py filters the input VCF to begin with the first phased variant (see filter_vcf()). When vcf2fasta treats a VCF as "phased," it treats unphased heterozygous genotypes as phased and arbitrarily assigns each allele to a haplotype. This means that unphased heterozygous genotypes must be removed from the phased VCF prior to running vcf2fasta. 
